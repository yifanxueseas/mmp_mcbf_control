...
class Frame:
    ...
    def __init__(self, origin=(0.0, 0.0), angle=0.0) -> None:
        self.x = origin[0]
        self.y = origin[1]
        self.angle = angle
...
class Transform:
    ...
    @staticmethod
    def affine_transform(state: Union[list, np.ndarray], rotation: float, translation:Optional[tuple]=None, scale=1.0) -> np.ndarray:
...
class ScaleOffsetReverseTransform:
    def __init__(self, scale=1.0, offsetx_after=0.0, offsety_after=0.0, x_reverse=False, y_reverse=False, x_max_before=0, y_max_before=0):
    ...
    def cvt_state(self, state: Union[list, np.ndarray], forward=True) -> Union[list, np.ndarray]:
        """Return the transformed state. If forward=False, it means from the transformed state to the original one.
        """
